...
# Group by name with polarization removed, e.g. "case5-thinDSP_pp_SHG" -> "case5-thinDSP_SHG"
POL_PATTERN = re.compile(r'_(ss|sp|pp)_', re.IGNORECASE)

def get_base_name(dn):
    return POL_PATTERN.sub('_', dn)

def get_polarization(dn):
    m = POL_PATTERN.search(dn)
    return m.group(1) if m else 'unknown'

from collections import defaultdict
groups = defaultdict(list)
for dn in all_dirs:
    groups[get_base_name(dn)].append(dn)

# Get user input for averaging window and plotstyle 
while True:
    try:
...
while True:
    plotstyle = input('Please enter a plot style ("log", "counts", "column norm", or "specular linecut" \n> ')
    if plotstyle in ['log', 'counts', 'column norm', 'specular linecut']:
        break
    print("Invalid input. Please enter 'y' or 'n'.")


for base_name, dir_list in groups.items():
    specular_results = {}  # polarization -> (expected_ky_sorted, specular_counts)

    for dn in sorted(dir_list):
        data_name = dn
        polarization = get_polarization(dn)
        ...
        def find_x_center_from_counts(counts):
            ...
            skip_at_both_ends = 490 # Don't consider the first N or last N columns 
            column_sums = np.nansum(counts[:, skip_at_both_ends : 1024 - skip_at_both_ends], axis=0) 
            return int(np.nanargmax(column_sums)) + skip_at_both_ends
        ...
        if plotstyle == 'specular linecut':
                band_half_width = 0.05 # in ky units 
                specular_counts = np.array([
                    np.nanmean(Z[np.abs(reflected_ky_axis_common - kx) <= band_half_width, i])
                    for i, kx in enumerate(expected_ky_sorted)
                ])
                specular_results[polarization] = (expected_ky_sorted, specular_counts)
    
        else:
            # ... existing if/elif for log, counts, column norm ...
    ...
    if plotstyle == 'specular linecut' and specular_results:
        ...
        plt.tight_layout()
        out_png = os.path.join(os.getcwd(), base_name + "-specular_linecut.png")
        plt.savefig(out_png, dpi=300)
        plt.show()
